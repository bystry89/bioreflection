...
#session 1
part1 <- read.csv("data/01a_raw_data.csv")

#code politics
part1 <- part1[3:nrow(part1),] %>% 
  mutate(RL = if_else(STUDY_ID %in% c("6062f4aff386aeb61a0ae890", "606447a465eee832c645552f"), "Right", "Left")) 

#check for duplicates
dupl <- part1 %>% group_by(PROLIFIC_PID) %>% dplyr::summarise(n=n()) %>% 
  filter(n>1) %>% .$PROLIFIC_PID

#filter duplicates
part1_filt <- part1 %>% filter(!(PROLIFIC_PID %in% dupl))
  
#session 2 data
part2 <- read.csv("data/01b_raw_data.csv")
part2 <- part2[3:nrow(part2),]

part2_filt <- part2 %>% filter(!(PROLIFIC_PID %in% dupl))
...
tlong_full <- tlong %>% inner_join(tlong2, by = c("PROLIFIC_PID", "key"))

reflection <- part2_filt %>% select(ab_f_ref_1, su_f_ref_1,
                                    va_f_ref_1, gm_f_ref_1,
                                    sa_f_ref_1, br_f_ref_1, 
                                    pb_f_ref_1, su_n_ref_1,
                                    ab_n_ref_1, va_n_ref_1,
                                    gm_n_ref_1, sa_n_ref_1,
                                    br_n_ref_1, pb_n_ref_1,
                                    PROLIFIC_PID) %>%
  gather(-PROLIFIC_PID, key = 'key', value = 'resp') %>% filter(resp != '') %>% 
  left_join(select(part2_filt, issue, Q75_Page.Submit, explain, PROLIFIC_PID, c_ref_1), 
            by = 'PROLIFIC_PID') %>% 
...
write.csv(tlong_full, 'data/01_long_data.csv')
