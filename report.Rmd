...
Finally, we fitted an autoregression model to check whether response to the normative question regarding the target issue given in session 2 was predicted by the factual opinion following the reflection independently of the normative response from session 1. It didn't ($\beta$ = `r round(summ(m1_ref_4)$coeftable[3,1], 2)`, *t*(`r round(summ(m1_ref_4)$coeftable[3,4], 0)`) = `r round(summ(m1_ref_4)$coeftable[3,3],2)`, *p* `r myround2(summ(m1_ref_4)$coeftable[3,5], 2)`).
...
Finally, we fitted an autoregression model to check whether response to the factual question regarding the target issue given in session 2 was predicted by the normative opinion following the reflection independently of the factual response from session 1. I did ($\beta$ = `r round(summ(m1_rat_4)$coeftable[3,1], 2)`, *t*(`r round(summ(m1_rat_4)$coeftable[3,4], 0)`) = `r round(summ(m1_rat_4)$coeftable[3,3],2)`, *p* `r myround2(summ(m1_rat_4)$coeftable[3,5], 3)`).
...
We also check if factual beliefs for target issues are less dispersed in session 1 than in session 2. We didn't observe any evidence of such a systematic tendency ($\beta$ = `r round(summ(m2_f_dev)$coeftable[2,1], 2)`, *t*(`r round(summ(m2_f_dev)$coeftable[2,4], 0)`) = `r round(summ(m2_f_dev)$coeftable[2,3],2)`, *p* `r myround2(summ(m2_f_dev)$coeftable[2,5], 3)`)
